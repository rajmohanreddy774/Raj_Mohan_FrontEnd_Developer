...
router.get("/get-all-capsules-count", isAuthenticatedUser, async (req, res) => {
  ...
  try {
    ...
    console.log(error);
    return res.status(501).send({ msg: error.msg });
  }
});
...
router.post("/get-filtered-capsules", isAuthenticatedUser, async (req, res) => {
  ...
  try {
    ...
    if (selectedStatus && selectedType) {
      ...
      modifiedApi = process.env.API + `?status=${selectedStatus}`;
    } else if (selectedType) {
      modifiedApi = process.env.API + `?type=${selectedType}`;
    } else if (selectedDate) {
      modifiedApi = process.env.API + `?original_launch=${selectedDate}`;
    }
